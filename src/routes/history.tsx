          ...
          {sessions.isLoading ? (
            <div className="space-y-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className="h-16 rounded-2xl bg-surface-card border border-border skeleton"
                />
              ))}
            </div>
            ...
            <StaggerContainer className="space-y-2">
              {sessions.data!.map((s: any) => (
                <StaggerItem key={s.id}>
                  <Link
                    to={`/workout/${s.id}`}
                    className="flex items-center gap-3 rounded-2xl bg-surface-card border border-border p-3.5 transition-all hover:-translate-y-0.5 hover:shadow-md active:scale-[0.98]"
                  >
