...
function ProfilePage() {
  ...
  const displayName =
    me.data?.profile?.full_name || user?.user_metadata?.full_name || mockUser.name;
      ...
      <PageTransition>
        <main className="flex-1 space-y-4 px-5 py-4">
            ...
            <Card variant="default" className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    className={`flex h-10 w-10 items-center justify-center rounded-xl ${whatsappOk ? "bg-success/15 text-success" : "bg-warning/15 text-warning"}`}
                  >
                    {whatsappOk ? (
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-text-tertiary">
                      {whatsappOk ? "Verificado ✓" : "Não verificado"}
                    </p>
                  </div>
                </div>
                {!whatsappOk && (
          ...
          {!session ? (
            <Link
              to="/auth"
              className="flex items-center justify-center gap-2 rounded-2xl bg-primary py-3.5 font-semibold text-primary-foreground shadow-lg shadow-primary/20"
            >
