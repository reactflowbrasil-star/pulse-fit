...
function AdminDashboard() {
  ...
  const messages = useQuery({ queryKey: ["wa", "messages", "admin"], queryFn: () => listWhatsappMessages({ data: { limit: 20 } }), refetchInterval: 10000 });
  ...
  const broadcastMut = useMutation({
    ...
    onSuccess: (r) => { setFeedback(r.ok ? `Broadcast: ${r.sent} enviadas` : r.error); if (r.ok) setBroadcast(""); },
  });
      ...
      <PageTransition>
        <main className="flex-1 space-y-4 px-5 py-4 overflow-y-auto">
          <StaggerContainer className="grid grid-cols-3 gap-2">
            <StaggerItem><Card variant="default" className="p-3 text-center"><Users className="h-4 w-4 mx-auto mb-1 text-primary" /><p className="font-display text-xl font-bold">{total}</p><p className="text-[10px] text-text-tertiary">Usuários</p></Card></StaggerItem>
            <StaggerItem><Card variant="default" className="p-3 text-center"><MessageCircle className="h-4 w-4 mx-auto mb-1 text-success" /><p className="font-display text-xl font-bold">{verified}</p><p className="text-[10px] text-text-tertiary">Verificados</p></Card></StaggerItem>
            <StaggerItem><Card variant="default" className="p-3 text-center"><div className={`h-2.5 w-2.5 rounded-full mx-auto mb-1.5 ${status.data?.state === "open" ? "bg-success" : "bg-destructive"}`} /><p className="font-display text-lg font-bold">{status.data?.state ?? "…"}</p><p className="text-[10px] text-text-tertiary">Bot</p></Card></StaggerItem>
          </StaggerContainer>

          {/* NVIDIA Panel Link */}
          <Card variant="gradient" hover className="p-4 cursor-pointer" onClick={() => navigate({ to: "/admin/nvidia" })}>
          ...
          <Card variant="default" className="p-4 space-y-3">
            ...
            <p className="text-xs text-text-tertiary">Enviar para {verified} usuários verificados.</p>
            <textarea className="min-h-[70px] w-full rounded-2xl bg-surface-elevated border border-border px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/40 placeholder:text-text-muted" placeholder="Sua mensagem..." value={broadcast} onChange={(e) => setBroadcast(e.target.value)} maxLength={4000} />
            <Button className="w-full" onClick={() => broadcast.trim() && broadcastMut.mutate(broadcast.trim())} loading={broadcastMut.isPending} disabled={!broadcast.trim()}>
